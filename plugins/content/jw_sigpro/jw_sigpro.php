    ...
    public function renderSIGP(&$row, &$params, $page = 0)
    {
        ...
        if (preg_match_all($regex, $row->text, $matches, PREG_PATTERN_ORDER) > 0) {
            ...
            foreach ($matches[0] as $key => $match) {
                $src_tagcontent = preg_replace("/{.+?}/", "", $match);
                $tagcontent = trim(strip_tags($src_tagcontent));
                ...
                if (strpos($tagcontent, 'www.flickr.com') === false) {
                    ...
                    $flickrRegex = "#flickr\.com/photos/(.*?)/(sets|albums|galleries)/(.*)#s";
                    ...
                    if (preg_match_all($flickrRegex, $flickrSetUrl, $flickrMatches, PREG_PATTERN_ORDER) > 0) {
                        ...
                        if ($isFlickrGallery) {
                            $flickrPhotoData = $getFlickrData->photos;
                        } else {
                            $flickrPhotoData = $getFlickrData->photoset;
                        }

                        $flickrSetTitle = (!empty($flickrPhotoData->title)) ? $flickrPhotoData->title : '';

                        // Initiate array to hold gallery
                        $gallery = array();

                        $galleryData = @$flickrPhotoData->photo;
                        ...
                        foreach ($galleryData as $key => $photo) {
                            ...
                            if ($gal_captions == 2) {
                                ...
                                $gallery[$key]->captionDescription = $photo->description.' - '.JText::_('JW_SIGP_PLG_FLICKR_PUBLISHED_IN').' <a href="'.$flickrSetUrl.'">'.$flickrSetTitle.'</a> '.JText::_('JW_SIGP_PLG_FLICKR_BY').' <a target="_blank" href="https://www.flickr.com/photos/'.$flickrUsername.'">'.$flickrUsername.'</a>';
                            } elseif ($gal_captions == 1) {
                                $gallery[$key]->captionTitle = '';
                                $gallery[$key]->captionDescription = JText::_('JW_SIGP_PLG_FLICKR_GENERIC_DESCRIPTION').' <a target="_blank" href="'.$flickrSetUrl.'">'.$flickrSetTitle.'</a> '.JText::_('JW_SIGP_PLG_FLICKR_BY').' <a target="_blank" href="https://www.flickr.com/photos/'.$flickrUsername.'">'.$flickrUsername.'</a>';
                            } else {
                                $gallery[$key]->captionTitle = '';
                            ...
                            if ($downloadFile) {
                                $gallery[$key]->downloadLink = SimpleImageGalleryProHelper::replaceHtml('<br /><a class="sigProDownloadLink" target="_blank" href="'.$photo->url_o.'" download>'.JText::_('JW_SIGP_PLG_FLICKR_DOWNLOAD_IMG').'</a>');
                            } else {
                ...
                if (JRequest::getCmd('format') == '' || JRequest::getCmd('format') == 'html') {
                    ...
                    if (version_compare(JVERSION, '1.6.0', 'lt')) {
                        JHTML::_('behavior.mootools'); // Enforce Mootools first in Joomla 1.5
                    }
                    if (version_compare(JVERSION, '1.6.0', 'ge') && version_compare(JVERSION, '3.0.0', 'lt')) {
                        JHTML::_('behavior.framework'); // Enforce Mootools first in Joomla 1.6-2.5
                    }
                    ...
                    if (version_compare(JVERSION, '3.0.0', 'ge')) {
                        ...
                        if (strpos($jQueryHandling, '1.') !== false) {
                            $jQueryHandling = 'googlecdn'; // Fallback fix for SIGPro versions before v3.0.7
                        }
                        if ($jQueryHandling) {
                ...
                // Do the replace
                $row->text = preg_replace("#{".$this->plg_tag."}".$src_tagcontent."{/".$this->plg_tag."}#s", $plg_html, $row->text);
            } // end foreach
