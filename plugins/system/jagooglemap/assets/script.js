...
JAWidgetMap = new Class({
	...
	displayMap: function () {
		...
		for(var i in locations.location) {
            ...
            var openTooltip = locations.tooltip && locations.tooltip[i] ? true : false;
			...
			if(this.isLatLon(latitude) && this.isLatLon(longitude)){
				...
				mapmarker.push(this.showLocation2(latitude, longitude, info, icon, openTooltip));
			}else{
				this.showLocation(location, info, openTooltip);
			}
		}
	...
	showLocation: function (address, info, openTooltip) {
		this.hideRoute();
		// hide route
	...
	showLocation2: function (lat, lon, info, icon, openTooltip) {
		this.hideRoute();
