...
class ModJAMastheadHelper
{
    ...
    public function getMasthead($params)
    {
        ...
        if(!empty($config_menuItem->item_id) && is_object($config_menuItem->item_id)){
            $config_menuItem->item_id = json_decode(json_encode($config_menuItem->item_id),true);
        }
        $config_option = $params->get('option_config');
        $config = '';
        if(!empty($config_menuItem->item_id) && count($config_menuItem->item_id)){
            for($i = 0; $i <= count($config_menuItem->item_id); $i++){
                if(isset($config_menuItem->item_id[$i]) && count($config_menuItem->item_id[$i]))
                {
                    foreach($config_menuItem->item_id[$i] as $k => $idx){
                        ...
                        $menu_background='';
                        $menu_desc='';
                        if (!empty($config_menuItem->title->{$i})) {
                        	$menu_title=$config_menuItem->title->{$i};
                        } else if (!empty($config_menuItem->title[$i])) {
                        	$menu_title=$config_menuItem->title[$i];
                        }

                        if (!empty($config_menuItem->background->{$i})) {
                        	$menu_background=$config_menuItem->background->{$i};
                        } else if (!empty($config_menuItem->background[$i])) {
                        	$menu_background=$config_menuItem->background[$i];
                        }

                        if (!empty($config_menuItem->description->{$i})) {
                        	$menu_desc=$config_menuItem->description->{$i};
                        } else if (!empty($config_menuItem->description[$i])) {
                        	$menu_desc=$config_menuItem->description[$i];
                        }
