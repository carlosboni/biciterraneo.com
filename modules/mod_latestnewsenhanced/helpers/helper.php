...
// 		}
	...
	*/
	static function getHtmlATag($module, $item, $follow = true, $tooltip = true, $popup_width = '600', $popup_height = '500', $css_classes = '', $anchors = '')
	{
	    return self::getATag($item, $follow, $tooltip, $popup_width, $popup_height, $css_classes, $anchors, $module->id);
	}

	/*
	 * for backward compatibility
	 */
	static function getATag($item, $follow = true, $tooltip = true, $popup_width = '600', $popup_height = '500', $css_classes = '', $anchors = '', $module_id = 0)
	{
		$attribute_title = '';
		$attribute_class = '';
		...
		switch ($item->linktarget) {
			...
			case 3:	// open in a modal window
				//JHtml::_('behavior.modal', 'a.lnemodal');
				$extra_url = '';
				if ($item->isinternal) {
				...
				return '<a href="'.$item->link.$extra_url.$anchors.'"'.$attribute_class.$attribute_title.' onclick="return false;" data-modaltitle="'.htmlspecialchars($item->linktitle, ENT_COMPAT, 'UTF-8').'" data-toggle="modal" data-target="#lnemodal_'.$module_id.'">';
	...
	static function getDetails($params, $prefix = '')
	{
		...
		while ($params->get($prefix.'info_'.$j) != null) {
			if ($params->get($prefix.'info_'.$j, 'none') != 'none') {
				...
				$details[] = ''; // icon, although we don't give the choice
				$details[] = $params->get($prefix.'extra_classes_'.$j, '');
	...
	static function getInfoBlock($infos, $params, $item, $item_params = null)
	{
		...
		$info_block .= '<dt>'.JText::_('MOD_LATESTNEWSENHANCEDEXTENDED_INFORMATION_LABEL').'</dt>';
			...
			switch ($value[0]) {
				...
				case 'readmore':
					...
					if (isset($item->link) && !empty($item->link) && $item->cropped) {
						...
						$info_block .= '<span class="detail detail_readmore' . $value[4] . '">';
				...
				case 'hits':
					...
					if (isset($item->hits)) {
						...
						$info_block .= '<span class="detail detail_hits' . $value[4] . '">';
				...
				case 'rating':
					...
					if (isset($item->vote)) {
						...
						$info_block .= '<span class="detail detail_rating' . $value[4] . '">';
				...
				case 'author':
					...
					if (isset($item->author)) {
						...
						$info_block .= '<span class="detail detail_author' . $value[4] . '">';
				...
				case 'keywords':
					if (isset($item->metakey) && !empty($item->metakey)) {
						...
						$info_block .= '<span class="detail detail_keywords' . $value[4] . '">';
				...
				case 'linkedcategory':
					...
					if (isset($item->category_title)) {
						...
						$info_block .= '<span class="detail detail_category' . $value[4] . '">';
...
// 					} else {
					if (isset($item->date)) {
						...
						$info_block .= '<span class="detail detail_date' . $value[4] . '">';
				...
				case 'time':
					if (empty($item->date)) {
						...
						$info_block .= '<span class="detail detail_time' . $value[4] . '">';
