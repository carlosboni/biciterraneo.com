...
<?php if ($datasource != 'articles') : ?>
	<div class="<?php echo $alert_error_classes; ?>"><?php echo JText::_('MOD_LATESTNEWSENHANCEDEXTENDED_ERROR_WRONGLAYOUT'); ?></div>
...
<?php else : ?>
				...
				<li class="latestnews-item id-<?php echo $item->id; ?> catid-<?php echo $item->catid; ?><?php echo $css_active; ?><?php echo $css_limited; ?><?php echo $css_shadow; ?><?php echo $css_featured; ?>">
					<?php if ($show_errors && !empty($item->error)) : ?>
						<div class="alert <?php echo SYWUtilities::getBootstrapProperty('alert-error', $bootstrap_version); ?><?php echo ($bootstrap_version > 2) ? ' alert-dismissible' : '' ?>"<?php echo ($bootstrap_version > 2) ? ' role="alert"' : '' ?>>
                			<button type="button" class="close" data-dismiss="alert" aria-label="<?php echo JText::_('MOD_LATESTNEWSENHANCEDEXTENDED_CLOSE'); ?>"><span aria-hidden="true">&times;</span></button>
								...
								<div class="newsinfooverhead">
									...
									<?php if ($show_title) : ?>
										<h<?php echo $title_html_tag; ?> class="newstitle">
											<?php if ($link_title) : ?>
												<?php if ($item->link) : ?>
													<?php echo modLatestNewsEnhancedExtendedHelper::getHtmlATag($module, $item, $follow, $link_tooltip, $popup_width, $popup_height); ?>
								...
								<?php if ($item->text) : ?>
									<div class="newsintro">
										...
										<?php if ($append_readmore && $link_label_item && $item->cropped) : ?>
											<?php if ($item->link) : ?>
												<?php echo modLatestNewsEnhancedExtendedHelper::getHtmlATag($module, $item, $follow, $link_tooltip, $popup_width, $popup_height, 'link_append'); ?>
								...
								<?php if ($add_readmore && $link_label_item && $item->cropped) : ?>
									<?php if ($item->link) : ?>
										<p class="link<?php echo $extrareadmoreclass; ?>">
											<?php echo modLatestNewsEnhancedExtendedHelper::getHtmlATag($module, $item, $follow, $link_tooltip, $popup_width, $popup_height, $extrareadmorelinkclass); ?>
												<span><?php echo $link_label_item; ?></span>
											</a>
										</p>
									<?php endif; ?>
	...
	<?php
        if ($modal_needed) {
    		$layout = new JLayoutFile('lnemodal', JPATH_ROOT.'/modules/mod_latestnewsenhanced/layouts'); // no overrides possible
