...
class SigProModelGallery extends SigProModel
{
    public function getData()
    {
        ...
        if (!$folder || !JFolder::exists($path.'/'.$folder)) {
            $app->redirect('index.php?option=com_sigpro', JText::_('COM_SIGPRO_THE_SPECIFIED_FOLDER_DOES_NOT_EXIST'), 'error');
        }
    ...
    public function save()
    {
        ...
        foreach ($files as $key => $file) {
            $title = preg_replace("#(\r\n|\r|\n|\t|\s+)#s", ' ', $titles[$key]);
            $description = preg_replace("#(\r\n|\r|\n|\t|\s+)#s", ' ', $descriptions[$key]);
            if (JFile::exists($path.'/'.$folder.'/'.$file)) {
                $labels .= $file.'|'.$title.'|'.$description."\n";
            }
        }
