...
function modChrome_T3Section($module, &$params, &$attribs) { 
	...
	// Background Module
	$bgPosition = $params->get('align-bg');
	$modBg = '';
	$classBg = '';

	if($params->get('mod-bg')) {
		$classBg = 'has-bg';
		$modBg = 'style="background-image:url('.JUri::root().''.$params->get('mod-bg').');background-position: center '.$bgPosition.';"';
	}
	...
	if($params->get('padding-top')) {
		$spaceTop    = 'padding-top';
	}

	// Space Bottom
	$spaceBottom = '';
	if($params->get('padding-bottom')) {
		$spaceBottom    = 'padding-bottom';
	}

	// Align Title
	$alignTitle = '';
	if($params->get('align-title')) {
		$alignTitle    = $params->get('align-title');
	}

	// Sub Heading
	$subHeading = '';

	if($params->get('sub-heading') && ($moduleType != 'mod_ja_acm')) {
		$subHeading    = '<div class="sub-heading"><span>'.$params->get('sub-heading').'</span></div>';
	}

	// DT Calendar Description
	$dtDescription = '';
	if($params->get('dt-description')) {
		$dtDescription = '<span>'.$params->get('dt-description').'</span>';
	}

	// Special Module
	$specialMd = '';
	if($moduleType == 'mod_articles_latest') {
		$specialMd = 'special-module';
	}

	if (!empty ($module->content)) {
		$html = "<{$moduleTag} class=\"t3-section module{$moduleClassSfx} {$moduleClass} {$spaceTop} {$spaceBottom} {$classBg} {$specialMd}\" id=\"Section{$module->id}\" {$modBg}>" .
					"<div class=\"{$moduleWidth}\"><div class=\"module-inner title-{$alignTitle}\"><div class=\"module-title-wrap\">{$subHeading}";

		if (($module->showtitle != 0) && ($moduleType != 'mod_ja_acm')) {
			$html .= "<{$headerTag} class=\"module-title {$headerClass}\">{$module->title}{$dtDescription}</{$headerTag}>";
		}

		$html .= "</div><div class=\"module-ct\">{$module->content}</div></div></div></{$moduleTag}>";
